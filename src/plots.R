...
# p: parameter used to build the influence matrix A.
# folder: folder in which to generate the plots.
#############################################################################################
measure.histo <- function(vals, measure="Interest Centrality", p, folder)
{	data <- data.frame(Centrality=vals)
	...
	plt <- plt + ggtitle(paste(measure," histogram for p=",p,sep=""))
	plt <- plt + xlab(measure) 
	plt <- plt + ylab("Count")
	
	plot.filename <- paste(folder,"/histo_meas=",measure,"_p=",p,".pdf",sep="")
...
# p.vals: parameters used to build the influence matrix A.
# measure: name of the alternative measure.
# folder: folder in which to generate the plots.
#############################################################################################
corr.plot <- function(cor.vals, p.vals, measure, folder)
{	data <- data.frame(p=p.vals,Correlation=cor.vals)
print(data)	
	plt <- ggplot(data=data, aes(x=p,y=Correlation)) 
...
# folder: folder in which to generate the plots.
#############################################################################################
rank.diff.lineplot <- function(ref.vals, comp.vals, ref.measure, comp.measure="Interest Centrality", p, folder)
{	data <- data.frame(number.nodes=c(1:number.nodes),Ranking.difference=sort(rank(comp.vals)-rank(ref.vals)))
	...
	plt <- plt + geom_point(size=4,colour="steelblue")
	plt <- plt + geom_line(size=1,colour="steelblue")
	plt <- plt + ggtitle(paste("Nodes sorted by rank difference with ",measure," for p=",p,sep=""))
	...
	plot.filename <- paste(folder,"/rank_lineplot_meas=",measure,"_p=",p.vals[i],".pdf",sep="")
	pdf(file=plot.filename,bg="white")
	print(plt)
	dev.off()
}
...
# folder: folder in which to generate the plots.
#############################################################################################
rank.diff.barplot <- function(ref.vals, comp.vals, ref.measure, comp.measure="Interest Centrality", p, folder)
{	ref.rk <- rank(ref.vals,ties.method="min")
	...
	plot.filename <- paste(folder,"/rank_barplot_meas=",measure,"_p=",p,".pdf",sep="")
	pdf(file=plot.filename,bg="white")
	print(plt)
	dev.off()
}
