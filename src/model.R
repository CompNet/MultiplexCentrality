...
# grad.step.size: gradient step size.
#############################################################################################
process.opinion.centrality <- function(A, network, alpha, budget, b, grad.horizon=1000, grad.step.size=0.001)
{	number.layers<-length(network)
	...
	y <- Sol(constraints.matrix,b.layer,number.layers,number.nodes,c1,budget,x0,lambda1,lambda2,lambda3,grad.horizon,grad.step.size)
