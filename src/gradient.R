...
#############################################################################################
Sol.v1 <- function(A, number.layers, number.nodes, u, d, x, lambda1, lambda2, lambda3, grad.horizon)#, npar=TRUE, print=TRUE)
{	l <- number.nodes
...
# returns: the minimum of 1^tx-lambda1\sum_i[log((b-Ax)_i)]+lambda2\sum_i[(cx-d)_i]^2-\lambda3[log(1-1^tx)].
#############################################################################################
Sol.v2 <- function(A, number.layers, number.nodes, u, d, x, lambda1, lambda2, lambda3, grad.horizon)#, npar=TRUE, print=TRUE)
{	l <- number.nodes
	...
	# update of the gradient scheme
	for(k in 1:(grad.horizon-1))
	{	Result[,k+1] <- Result[,k]+(1/k)*(B%*%Result[,k]-lambda1*array((sum(Result[,k])-d),c(l,1)))
