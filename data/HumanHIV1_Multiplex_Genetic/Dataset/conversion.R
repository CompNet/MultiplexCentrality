library("igraph")
folder <- "data/HumanHIV1_Multiplex_Genetic/"

edge.file <- paste(folder,"Dataset/humanHIV1_genetic_multiplex.edges",sep="")
edge.list <- as.matrix(read.table(edge.file))

node.file <- paste(folder,"Dataset/humanHIV1_genetic_nodes.txt",sep="")
node.list <- as.matrix(read.table(node.file, header=TRUE))

layer.file <- paste(folder,"Dataset/humanHIV1_genetic_layers.txt",sep="")
...
#	g <- simplify(graph=g, remove.multiple=TRUE)
	...
	cat("D:\\\\HumanHIV1_Multiplex_Genetic\\\\MuxViz\\\\",layer.name,".edgelist;",layer.name,";D:\\\\HumanHIV1_Multiplex_Genetic\\\\MuxViz\\\\",layer.name,".nodes\n",sep="")
}

# record as R object
print(HumanHIV1)
data.file <- paste(folder,"HumanHIV1.Rdata",sep="")
save(HumanHIV1, file=data.file)
