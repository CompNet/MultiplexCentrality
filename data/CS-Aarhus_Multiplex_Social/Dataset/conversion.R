library("igraph")
folder <- "data/CS-Aarhus_Multiplex_Social/"

edge.file <- paste(folder,"Dataset/CS-Aarhus_multiplex.edges",sep="")
edge.list <- as.matrix(read.table(edge.file))

node.file <- paste(folder,"Dataset/CS-Aarhus_nodes.txt",sep="")
node.list <- as.matrix(read.table(node.file, header=TRUE))

layer.file <- paste(folder,"Dataset/CS-Aarhus_layers.txt",sep="")
