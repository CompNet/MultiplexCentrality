library("igraph")
folder <- "data/Rattus_Multiplex_Genetic/"

edge.file <- paste(folder,"Dataset/rattus_genetic_multiplex.edges",sep="")
edge.list <- as.matrix(read.table(edge.file))

node.file <- paste(folder,"Dataset/rattus_genetic_nodes.txt",sep="")
node.list <- as.matrix(read.table(node.file, header=TRUE))

layer.file <- paste(folder,"Dataset/rattus_genetic_layers.txt",sep="")
...
#	g <- simplify(graph=g, remove.multiple=TRUE)
	...
	cat("D:\\\\Rattus_Multiplex_Genetic\\\\MuxViz\\\\",layer.name,".edgelist;",layer.name,";D:\\\\Rattus_Multiplex_Genetic\\\\MuxViz\\\\",layer.name,".nodes\n",sep="")
}

# record as R object
print(Rattus)
data.file <- paste(folder,"Rattus.Rdata",sep="")
save(Rattus, file=data.file)
